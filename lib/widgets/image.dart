...
void _paintImage({
  ...
  final PdfPoint inputSize = PdfPoint(image.width.toDouble(), image.height.toDouble());
  fit ??= BoxFit.scaleDown;
  final FittedSizes fittedSizes = applyBoxFit(fit, PdfPoint(inputSize.x / scale, inputSize.y / scale), outputSize);
  final PdfPoint sourceSize = PdfPoint(fittedSizes.source.x * scale, fittedSizes.source.y * scale);
  ...
  final PdfRect destinationRect = PdfRect.fromPoints(destinationPosition, destinationSize);
...
class Image extends Widget {
  ...
  void layout(Context context, BoxConstraints constraints, {bool parentUsesSize = false}) {
    final double w = width ?? (constraints.hasBoundedWidth ? constraints.maxWidth : constraints.constrainWidth(image.width.toDouble()));
    final double h = height ?? (constraints.hasBoundedHeight ? constraints.maxHeight : constraints.constrainHeight(image.height.toDouble()));

    final FittedSizes sizes = applyBoxFit(fit, PdfPoint(image.width.toDouble(), image.height.toDouble()), PdfPoint(w, h));
...
class Shape extends Widget {
  ...
  @override
  void layout(Context context, BoxConstraints constraints, {bool parentUsesSize = false}) {
    final double w = constraints.hasBoundedWidth ? constraints.maxWidth : constraints.constrainWidth(width);
    final double h = constraints.hasBoundedHeight ? constraints.maxHeight : constraints.constrainHeight(height);

    final FittedSizes sizes = applyBoxFit(fit, PdfPoint(width, height), PdfPoint(w, h));
    box = PdfRect.fromPoints(PdfPoint.zero, sizes.destination);
  }
