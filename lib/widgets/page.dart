...
class Page {
  Page({PageTheme pageTheme, PdfPageFormat pageFormat, BuildCallback build, ThemeData theme, PageOrientation orientation, EdgeInsets margin, bool clip = false})
      : assert(pageTheme == null || (pageFormat == null && theme == null && orientation == null && margin == null && clip == false), 'Don\'t set both pageTheme and other settings'),
        pageTheme = pageTheme ??
            PageTheme(
  ...
  void postProcess(Document document) {
    ...
    final EdgeInsets _margin = margin;
    BoxConstraints constraints = mustRotate ? BoxConstraints(maxWidth: pageFormat.height - _margin.vertical, maxHeight: pageFormat.width - _margin.horizontal) : BoxConstraints(maxWidth: pageFormat.width - _margin.horizontal, maxHeight: pageFormat.height - _margin.vertical);

    final ThemeData calculatedTheme = theme ?? document.theme ?? ThemeData.base();
    ...
    if (_build != null) {
      ...
      if (content != null) {
        ...
        if (_pdfPage.pageFormat.height == double.infinity) {
          ...
          constraints =
              mustRotate ? BoxConstraints(maxWidth: _pdfPage.pageFormat.height - _margin.vertical, maxHeight: _pdfPage.pageFormat.width - _margin.horizontal) : BoxConstraints(maxWidth: _pdfPage.pageFormat.width - _margin.horizontal, maxHeight: _pdfPage.pageFormat.height - _margin.vertical);
        }
      }
    }
  ...
  PdfPoint layout(Widget child, Context context, BoxConstraints constraints, {bool parentUsesSize = false}) {
    ...
    final double width = pageFormat.width == double.infinity ? child.box.width + _margin.left + _margin.right : pageFormat.width;

    final double height = pageFormat.height == double.infinity ? child.box.height + _margin.top + _margin.bottom : pageFormat.height;
