...
class PdfGraphics {
  ...
  void setFillColor(PdfColor color) {
    if (color is PdfColorCmyk) {
      PdfNumList(<double>[color.cyan, color.magenta, color.yellow, color.black]).output(buf);
      buf.putString(' k\n');
    } else {
  ...
  double _vectorAngle(double ux, double uy, double vx, double vy) {
    final double d = math.sqrt(ux * ux + uy * uy) * math.sqrt(vx * vx + vy * vy);
    if (d == 0.0) {
      return 0;
    }
  ...
  void _endToCenterParameters(double x1, double y1, double x2, double y2, bool large, bool sweep, double rx, double ry) {
    ...
    double dTheta = _vectorAngle((x1d - cxd) / rx, (y1d - cyd) / ry, (-x1d - cxd) / rx, (-y1d - cyd) / ry) % (math.pi * 2.0);
  ...
  void _bezierArcFromCentre(double cx, double cy, double rx, double ry, double startAngle, double extent) {
    ...
    double kappa = (4.0 / 3.0 * (1.0 - math.cos(halfFragment)) / math.sin(halfFragment)).abs();
